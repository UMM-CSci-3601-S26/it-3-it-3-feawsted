...
class TermsControllerSpec {
  ...
  void getTermsMergesAndSortsAllFields() {
    ...
    DistinctIterable<String> slStyleA = makeIterable(Arrays.asList("Wide Ruled"));
    DistinctIterable<String> slStyleO = makeIterable(Arrays.asList("College Ruled"));
    ...
    DistinctIterable<String> invStyle  = makeIterable(Arrays.asList("College Ruled", "Narrow Ruled"));
    ...
    when(supplyListCollection.distinct(eq("style.allOf"),   eq(String.class))).thenReturn(slStyleA);
    when(supplyListCollection.distinct(eq("style.anyOf"),   eq(String.class))).thenReturn(slStyleO);
    ...
    when(inventoryCollection.distinct(eq("style"),    eq(String.class))).thenReturn(invStyle);
    ...
    doAnswer(inv -> {
      ...
      assertEquals(Arrays.asList("College Ruled", "Narrow Ruled", "Wide Ruled"), terms.style);
      return null;
