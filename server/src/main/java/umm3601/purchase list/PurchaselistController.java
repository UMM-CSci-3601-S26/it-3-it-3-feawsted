...
import umm3601.purchaselistiPurchaselistsPurchaselistchaselistItem;
...
// Define the Purchaselistist class if it doesn't exist elsewhere

/**
 * Controller for handling Purchaselistist-related API routes.
 *
 * Routes include:
 * - GET /api/purchaselistist → list apurchaselistselist items (with optional filters)
 * - GET /api/purchaselistist/{id} → get a singpurchaselistselist item
 *
 * Purchaselistist is the core data model for tracking what supplies students, and
 * will be used
...
public class PurchaselistController implements Controller {
  ...
  private static final String API_PURCHASELISTIST = "/apurchaselistselists";
  // private static final String API_PURCHASELISTIST_PRINT = "/apurchaselistselists/print";
  // private static final String API_PURCHASELISTIST_BY_NAME = "/apurchaselistselists/student/{name}";
  // private static final String API_PURCHASELISTIST_FAMILY = "/apurchaselistselists/family/{guardianName}";
  // private static final String API_PURCHASELISTIST_BY_ID = "/apurchaselistselists/{id}";
  // private static final String API_PURCHASELISTIST_ITEM = "/apurchaselistselists/{id}/item/{index}";
  ...
  private final JacksonMongoCollection<Purchaselist> purchaselististCollection;
  ...
  public PurchaselistController(JacksonMongoCollection<Family> familyCollection,
      ...
      JacksonMongoCollection<Purchaselist> purchaselististCollection) {
    this.familyCollection = familyCollection;
    this.supplyListCollection = supplyListCollection;
    this.purchaselististCollectionpurchaselistselistCollection;
  ...
  public PurchaselistController(MongoDatabase db) {
    ...
    purchaselististCollection = JacksonMongoCollection.builder().build(
        db, "purchaselistists", Purchaselist.class, UuidRepresentation.STANDARD);
  ...
  public Purchaselist createPurchaselist(StudentInfo student, List<SupplyList> allSupplies) {
    ...
    Purchaselist purchaselistist = new Purchaselist();
    purchaselistist.studentName = student.name; // can't display last name, so maybe guardian name instead?
    purchaselistist.school = student.school;
    purchaselistist.grade = student.grade;
    purchaselistist.requestedSupplies = student.requestedSupplies;
    purchaselistipurchaselistselist = items;
    return purchaselistist;
  }

  // --- PRINT ROUTES (on-the-fly, not persisted) ---
  public void exportPurchaselistsPdf(Context ctx) {
    // Fetch your purchaselistist data
    List<SupplyList> pdfSupplies = expandHighSchoolSupplies(
        supplyListCollection.find().into(new ArrayList<>()));
    List<Purchaselist> purchaselistists = familyCollection.find()
        .into(new ArrayList<>())
        .stream()
    ...
    for (Purchaselist c : purchaselistists) {
        text.append("(")
        ...
        for (PurchaselistItem item : c.purchaselistist) {
            text.append("(")
    ...
    ctx.header("Content-Disposition", "inline; filename=purchaselistists.pdf");
    ctx.result(pdfBytes);
  }
//   // GET /api/purchaselistist/print — all students
//   public void printAllPurchaselists(Context ctx) {
//     List<SupplyList> allSupplies = supplyListCollection.find().into(new ArrayList<>());
//     List<Purchaselist> purchaselistists = familyCollection.find().into(new ArrayList<>()).stream()
...
//         ctx.contentType("application/pdf");
//         ctx.header("Content-Disposition", "attachment; filename=purchaselistists.pdf");
...
// }
  ...
  // GET /api/purchaselistist/student/{name} — single student by full name
  ...
  // GET /api/purchaselistist/family/{guardianName} — all students in a family
  ...
  //   }
  //   List<Purchaselist> purchaselistists = families.stream()
  //       .flatMap(f -> f.students.stream().map(s -> createPurchaselist(s, allSupplies)))
  //       .collect(Collectors.toList());
  //   ctx.json(purchaselistists);
  //   ctx.status(HttpStatus.OK);
  // }

  // --- DIGITAL DRIVE-DAY ROUTES (persisted to MongoDB) ---

  // POST /api/purchaselistist — snapshot all families into tpurchaselistselists collection
  public void generateDigitalPurchaselists(Context ctx) {
    purchaselististCollection.deleteMany(new Document());
    ...
    List<Purchaselist> purchaselistists = familyCollection.find().into(new ArrayList<>()).stream()
        .flatMap(f -> f.students.stream().map(s -> createPurchaselist(s, allSupplies)))
        .collect(Collectors.toList());
    purchaselististCollection.insertMapurchaselistselists);
    ctx.json(purchaselistists);
  ...
  // GET /api/purchaselistist — query stored digitpurchaselistselists (optional ?school= and
  // ?grade= filters)
  public void getStoredPurchaselists(Context ctx) {
    Bson filter = constructFilter(ctx);
    ctx.json(purchaselististCollection.find(filter).into(new ArrayList<>()));
    ctx.status(HttpStatus.OK);
  }
  ...
  // // GET /api/purchaselistist/{id} — get a single storpurchaselistselist by id
  // public void getStoredPurchaselistById(Context ctx) {
  //   String id = ctx.pathParam("id");
  //   Purchaselist purchaselistist;
  //   try {
  //     purchaselististpurchaselistselistCollection.find(Filters.eq("_id", new ObjectId(id))).first();
  //   } catch (IllegalArgumentException e) {
  //     throw new BadRequestResponse("Invalid purchaselistist ID.");
  //   }
  //   if (purchaselistist == null) {
  //     throw new NotFoundResponse("Purchaselist not found.");
  //   }
  //   ctx.json(purchaselistist);
  //   ctx.status(HttpStatus.OK);
  // }

  // // PATCH /api/purchaselistist/{id}/item/{index} — update a single item (completed,
  ...
  //     throw new NotFoundResponse("Purchaselist not found.");
  //   }
  //   if (index < 0 || index >= purchaselistipurchaselistselist.size()) {
  ...
  //   Purchaselist.PurchaselistItem item = purchaselistipurchaselistselist.get(index);
  ...
  //   purchaselististCollection.sapurchaselistselist);
  //   ctx.json(purchaselistist);
  //   ctx.status(HttpStatus.OK);
  // }
  ...
  public void addRoutes(Javalin server) {
    ...
    // server.get(API_PURCHASELISTIST_PRINT, this::printAllPurchaselists);
    // server.get(API_PURCHASELISTIST_BY_NAME, this::printPurchaselistByStudent);
    // server.get(API_PURCHASELISTIST_FAMILY, this::printPurchaselistsByFamily);

    // Digital drive-day routes (persisted)
    server.post(API_PURCHASELISTIST, this::generateDigitalPurchaselists);
    server.get(API_PURCHASELISTIST, this::getStoredPurchaselists);
    server.get("/purchaselistists/export/pdf", this::exportPurchaselistsPdf);

    // server.get(API_PURCHASELISTIST_BY_ID, this::getStoredPurchaselistById);
    // server.patch(API_PURCHASELISTIST_ITEM, this::updPurchaselistselistItem);
  }
}
