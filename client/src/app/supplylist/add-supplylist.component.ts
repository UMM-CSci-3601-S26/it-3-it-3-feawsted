...
export class AddSupplyListComponent implements OnInit {
  ...
  parseDescription(input: string): void {
    ...
    // ── Size ──────────────────────────────────────────────────────────────────
    // size is a plain string field — collect all matches and join with sep
    // so "wide ruled or college ruled" becomes "wide ruled | college ruled".
    ...
    // ── Style (matched before type so shared terms like "college ruled" stay here) ─
    const matchedStyles = this.allTermMatches(lower, this.terms.style);
    if (matchedStyles.length) {
      patch['style'] = matchedStyles.join(sep);
    }
  ...
  submitForm() {
    ...
    // Values containing '|' were parsed with "or" → anyOf.
    // Values with ',' (or a single value, no separator) → allOf.
    const toAttr = (val: string | null | undefined) => {
      if (!val || !val.trim()) {
        return { allOf: [], anyOf: [] };
      }
      if (val.includes('|')) {
